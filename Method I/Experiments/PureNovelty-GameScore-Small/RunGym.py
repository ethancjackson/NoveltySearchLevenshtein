...
results_file = open('./testing_results_30eps.txt', 'w')
for game_name in game_names:
    results_file.write('{}\n'.format(game_name))
    results_file.write('GameScore: {}\n'.format(gs_score_dict[game_name]))
    results_file.write('GameScore(Lifetimes): {}\n'.format(gs_lifetime_dict[game_name]))
    results_file.write('PureNovelty: {}\n'.format(rs_score_dict[game_name]))
    results_file.write('PureNovelty(Lifetimes): {}\n'.format(rs_lifetime_dict[game_name]))
results_file.flush()
results_file.close()
...
for game_name in game_names:
    ...
    print('PureNovelty Mean: {}'.format(rs_mean))
    print('PureNovelty StDev: {}'.format(rs_std))
    print()
...
for game_name in game_names:
    print(game_name)
    gs_mean = np.mean(gs_lifetime_dict[game_name])
    gs_std = np.std(gs_lifetime_dict[game_name])
    rs_mean = np.mean(rs_lifetime_dict[game_name])
    rs_std = np.std(rs_lifetime_dict[game_name])
    print('GameScore Mean: {}'.format(gs_mean))
    print('GameScore StDev: {}'.format(gs_std))
    print('PureNovelty Mean: {}'.format(rs_mean))
    print('PureNovelty StDev: {}'.format(rs_std))
    print()
    test_stat, p_value = ttest_ind(gs_lifetime_dict[game_name], rs_lifetime_dict[game_name])
    print('p-value: {}'.format(p_value))
    print()
